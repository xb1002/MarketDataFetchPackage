...
示例中演示了如何下载历史 K 线以及分别获取最新成交价与最新 Mark Price。其他如指数 K 线、标记价格 K 线、溢价指数 K 线、资金费率历史与未平仓量均通过同一个 `MarketDataClient` 入口暴露。
...
因此 `get_*_klines`、`get_latest_mark_price` 等方法都直接消费上述官方端点，输出与 Binance、Bybit 相同的 tuple 结构。
