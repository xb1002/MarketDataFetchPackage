...
@pytest.mark.network
@pytest.mark.integration
def test_latest_premium_index_matches_ccxt(parity_provider: CCXTProviderContext) -> None:
    if not parity_provider.case.supports_premium_series:
        pytest.skip("ccxt exchange lacks premium index endpoint")
    latest = _call_provider(
        parity_provider,
        lambda: parity_provider.source.get_latest_premium_index(parity_provider.case.symbol),
    )
    ccxt_series = _fetch_ccxt_kline(
        parity_provider,
        method_name="fetchPremiumIndexOHLCV",
    ...
    ccxt_point = ccxt_series[-1]
    assert abs(latest[0] - ccxt_point[0]) <= SNAPSHOT_TIME_TOL_MS
    _assert_decimal_close(latest[1], ccxt_point[4], context="latest premium index")
