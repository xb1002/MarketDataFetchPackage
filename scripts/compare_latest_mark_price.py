"""Compare latest mark price snapshots against CCXT."""
...
def main(targets: Iterable[str] | None = None) -> None:
    for case in iter_cases(targets):
        print(f"\n=== {case.name} latest mark price ===")
        source = case.source_factory()
        try:
            ts, value = source.get_latest_mark_price(SYMBOL)
        ...
        try:
            ...
            kline = exchange.fetchMarkOHLCV(case.ccxt_symbol, timeframe="1m", limit=1)[-1]
