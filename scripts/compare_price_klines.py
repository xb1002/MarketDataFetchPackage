"""Compare price klines from providers against CCXT."""
...
def main(targets: Iterable[str] | None = None) -> None:
    ...
    for case in iter_cases(targets):
        print(f"\n=== {case.name} price klines ===")
        source = case.source_factory()
        try:
            ours = list(source.get_price_klines(window))
        ...
        try:
            ...
            ccxt_klines = exchange.fetch_ohlcv(case.ccxt_symbol, timeframe="1m", limit=5)
            formatted = [(k[0], k[1], k[2], k[3], k[4], k[5]) for k in ccxt_klines]
