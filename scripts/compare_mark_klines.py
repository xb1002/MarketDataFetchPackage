"""Compare mark price klines from providers against CCXT when available."""
...
def main(targets: Iterable[str] | None = None) -> None:
    ...
    for case in iter_cases(targets):
        print(f"\n=== {case.name} mark price klines ===")
        source = case.source_factory()
        try:
            ours = list(source.get_mark_price_klines(window))
        ...
        try:
            ...
            if not hasattr(exchange, "fetchMarkOHLCV"):
                raise ccxt.NotSupported("fetchMarkOHLCV not supported")
            klines = exchange.fetchMarkOHLCV(case.ccxt_symbol, timeframe="1m", limit=5)
