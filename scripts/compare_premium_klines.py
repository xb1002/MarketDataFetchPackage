"""Compare premium index klines from providers against CCXT when available."""
...
def main(targets: Iterable[str] | None = None) -> None:
    ...
    for case in iter_cases(targets):
        print(f"\n=== {case.name} premium index klines ===")
        source = case.source_factory()
        try:
            ours = list(source.get_premium_index_klines(window))
        ...
        try:
            ...
            klines = exchange.fetchPremiumIndexOHLCV(case.ccxt_symbol, timeframe="1m", limit=5)
