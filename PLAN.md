...
- **Bitget**：新增 `BitgetUSDTPerpDataSource`，基于 `/api/mix/v1/market/candles`（`kLineType` = `market/index/mark`）获取历史价格/指数/标记 K 线，`/api/mix/v1/market/history-fundRate`、`/api/mix/v1/market/ticker`、`/api/mix/v1/market/mark-price`、`/api/mix/v1/market/funding-time` 与 `/api/mix/v1/market/open-interest` 实现其他接口。由于官方未提供溢价指数端点，实现会用 Mark Price 与指数价格差值构建最新/历史溢价数据，并在统一 live 测试中覆盖所有方法。
