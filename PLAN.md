...
- **Bitget**：新增 `BitgetUSDTPerpDataSource`，基于 `/api/mix/v1/market/candles`（`kLineType` = `market/index/mark/premium`）获取历史价格/指数/标记/溢价 K 线，`/api/mix/v1/market/history-fundRate`、`/api/mix/v1/market/ticker`、`/api/mix/v1/market/mark-price`、`/api/mix/v1/market/funding-time` 与 `/api/mix/v1/market/open-interest` 实现其他接口，并通过 `kLineType=premium` 提供的官方数据源完成溢价指数历史与最新快照。
