...
- **Bitget**：新增 `BitgetUSDTPerpDataSource`，基于 UTA V3 行情接口 `/api/v2/mix/market/candles`（`productType=umcbl`，`kLineType` = `market/index/mark/premium`）获取历史价格/指数/标记/溢价 K 线，`/api/v2/mix/market/history-fund-rate` 获取资金费率历史，其余最新行情接口暂沿用 `/api/mix/v1/market/ticker`、`/api/mix/v1/market/mark-price`、`/api/mix/v1/market/funding-time` 与 `/api/mix/v1/market/open-interest`，并通过 `kLineType=premium` 提供的官方数据源完成溢价指数历史与最新快照。
